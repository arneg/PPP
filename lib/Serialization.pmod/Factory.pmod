...
object generate_default_type(array|string type) {
...
OUTER: for (int i = 0; i < sizeof(a); i++) {
	...
	object sig;
	if (stringp(type)) type = String.trim_all_whites(type);
	switch (type) {
	case "\n":
	case "":
	case "|": continue OUTER;
	case "array":
	    ...
	    sig = .Types.OneTypedList(generate_default_type(a[i+1][1..<1]));
	    i++;
	    break;
	case "mapping":
	    if (!arrayp(a[i+1])) break;
	    ...
	    if (pos != -1) {
		sig = .Types.OneTypedMapping(generate_default_type(a[i+1][1..pos-1]),
					     generate_default_type(a[i+1][pos+1..<1]));
		i++;
	     }
	    break;
	case "string":
...
string get_type(object o, string fname) {
    program p = compile_string(sprintf("string get_type(___prog o) { return sprintf(\"%%O\", typeof(o->%s)); }", fname), "-",
			       Resolver(([ "___prog" : object_program(o) ])));
    return p()->get_type(o);
}

object generate_struct(object o, string type, void|function helper) {
    array(string) a = indices(o) - indices(object_program(o));
    mapping m = ([]);

    foreach (a;; string symbol) {
	if (functionp(o[symbol])) {
	    continue;
	}
	...
	m[symbol] = generate_default_type(get_type(o, symbol));
    }
