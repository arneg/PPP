...
void init(mapping vars) {
	...
	void callback(int error, string key) {
	    if (error) {
		debug("handling", 0, "Absolutely fatal: initing handler did not work!!!\n");
	    } else {
		set_inited(1);
	    }
	};
...
void unfriend(MMP.Uniform entity) {
	...
	if (error) {
	    debug("friendship", 0, "%O: unfriend(%O) failed.\n", this, entity);
	}
...
void friend(MMP.Uniform entity) {
    ...
    void callback(int error) {
	if (error) {
	    debug("friendship", 0, "%O: friend(%O) failed.\n", this, entity);
	} else {
...
void friend_symmetric(MMP.Uniform entity) {
    ...
    void cb(int error) {
	if (error) {
	    debug("friendship", 0, "offer_quiet failed in friend_symmetric(). very bad!!!!!!\n\n");
	} else {
	    friend(entity);
	}
    };
...
void offer(MMP.Uniform entity) {
    void cb(int error) {
	if (error) {
	    debug("friendship", "offer_quiet failed in offer(). very bad!!!!!!\n\n");
	} else {
