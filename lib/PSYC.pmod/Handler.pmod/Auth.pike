...
int filter_error_authentication(MMP.Packet p, mapping _v) {
    ...
    if (!has_index(m->vars, "_location")) {
	P1(("PSYC.Handler.Auth", "incomplete _error_invalid_authentication (_location is missing)\n"))
	return 0;
    }
	...
	PT(("Uni", "I was not able to get authentication for %s (claims to be %s).\n", location, source))
	...
	PT(("Handler.Auth", "_error_authentication even though we never requested one.\n"))
    }
...
int filter_notice_authentication(MMP.Packet p, mapping _v) {
    ...
    if (!has_index(m->vars, "_location")) {
	P1(("PSYC.Handler.Auth", "incomplete _notice_authentication (_location is missing)\n"))
	return 0;
    }
